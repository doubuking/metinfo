define(function(require, exports, module) {
	...
	$(".list-type-update a").click(function(){
		...
		if(nm){
			if(!$("input[name='id']:checked").length){
				common.metalert({html:METLANG.unselected});
				return false;
			}
	...
	$('button[data-toggle="popover"]').popover({
		content:function(){
			return '<button class="btn btn-danger btn-sm list-type-del" role="button" data-value="0" style="margin-right:5px;">'+METLANG.thoroughlyDeleting+'</button><a class="btn btn-default btn-sm listdeleteno" role="button" href="javascript:;">'+METLANG.cancel+'</a>';
		},
		html:true,
		placement:'top'
	});
