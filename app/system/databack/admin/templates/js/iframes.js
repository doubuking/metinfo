...
function htmxun(json, my, k, data, tp) {
	...
	if (my) {
		if(tp!=2)$('#htmlloading h3').html('<img src="' + own_tem + 'loadings.gif" style="position:relative; top:3px;" /> ' +user_msg['js54']+'[' + json.length + '/' + (k + 1) + ']');
...
function methtml(my, tp) {
	...
	if (hl == 0) {
		nxt.empty();
		nxt.append('<img src="' + own_tem + 'loadings.gif" style="position:relative; top:3px;" />' + user_msg['jsx11']);
		hl = 1;
		$.ajax({
			url: my.attr('href'),
			type: "POST",
...
function getchangexml(type, st, et) {
	...
	var heth = '220';
	var chart = new FusionCharts(own_tem + "/Chart/swf/FCF_Column2D.swf", "ChartId", wdth, heth);
...
function addsave(met, i) {
	met.after('<span id="loadtxt">' + '<img src=\"' + own_tem + 'loadings.gif\" style=\"position:relative; top:4px;\" />' + user_msg['js48'] + '</span>');
	...
	if (comnn) {
		if (comnn.is(':hidden')) {
			if (!tp) {
				...
				my.attr('src', own_tem + '/colum1nx2.gif');
				ifreme_methei();
			}
		} else {
			comnn.hide();
			my.removeClass('columnimgon');
			my.attr('src', own_tem + '/colum1nx.gif');
			comnn.each(function() {
...
function adddisplayimg(my) {
	$('#loadtxt').html('<img src=\"' + own_tem + 'loadings.gif\" style=\"position:relative; top:4px;\" />' + user_msg['js48']);
