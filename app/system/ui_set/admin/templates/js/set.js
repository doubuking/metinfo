define(function(require, exports, module) {
	...
	var themefunc = require('own_tem/js/func');//函数
	var up = require('own_tem/js/upload');//自定义上传组件
	var banner = require('own_tem/js/banner');/*大图轮播设置*/
	var ckeditor = require('own_tem/js/ckeditor');/*大图轮播设置*/
	/*提示文字*/
	require('own_tem/tips/css/simpletooltip.min.css');
	require('own_tem/tips/js/simpletooltip.min');
	function tipscl(position){
		...
		$('.v52fmbx dl').simpletooltip({position:position});
	}
	
	/*展开隐藏设置*/
	require('own_tem/js/jquery.rotate');//载入旋转插件
	function rotatecl(){
