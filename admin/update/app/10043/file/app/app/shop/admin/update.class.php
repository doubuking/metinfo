...
class update extends admin{
    ...
    {
        global $_M;
        $tablesql = file_get_contents(PATH_ALL_APP . "shop/admin/sql/{$this->name}_{$this->appver}_{$this->appno}_mysql.json");
    ...
    public function updateAppVer()
    {
        ...
        if(version_compare($result['ver'],$this->appver)<0){
            $query = "DELETE FROM {$_M['table']['applist']} WHERE no = '{$this->appno}' AND `ver`='{$result['ver']}'";
            DB::query($query);
            $query = "INSERT INTO  {$_M['table']['applist']} SET no = '{$this->appno}',ver = '{$this->appver}',m_name = '{$this->m_name}',m_class = '{$this->m_class}',m_action = '{$this->m_action}',appname = '{$this->appname}',target='{$this->target}',info  = '{$this->info }',mlangok = '{$this->mlangok}',depend = '{$this->depend}'";
            DB::query($query);
        }
    }
    ...
    public function setWebLang()
    {
        ...
        foreach ($_M['langlist']['web'] as $lang) {
            ...
            if ($lang['mark'] == 'cn' || $lang['mark'] == 'en') {
                $content = file_get_contents(PATH_ALL_APP . "shop/admin/sql/lang_web_{$this->appno}_{$lang['mark']}.json");
                $content = jsondecode($content);
                $this->lang_insert($content, $lang['mark']);
            }else{
                //其他语种
                $content = file_get_contents(PATH_ALL_APP . "shop/admin/sql/lang_web_{$this->appno}_cn.json");
                $content = jsondecode($content);
                $this->lang_insert($content, $lang['mark']);
            }
        }
    ...
    public function setAdminLang()
    {
        ...
        foreach ($_M['langlist']['admin'] as $lang) {
            if ($lang['mark'] == 'cn' || $lang['mark'] == 'en') {
                $content = file_get_contents(PATH_ALL_APP . "shop/admin/sql/lang_admin_{$this->appno}_{$lang['mark']}.json");
                $content = jsondecode($content);
                $this->lang_insert($content, $lang['mark']);
            }else{
                //其他语种
                $content = file_get_contents(PATH_ALL_APP . "shop/admin/sql/lang_admin_{$this->appno}_cn.json");
