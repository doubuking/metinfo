...
class temtool extends admin {
	...
	public function __construct() {
		...
		$this->tem = load::own_class('admin/class/tem','new');
	}
	...
	public function dotable_temlist_json() {
		...
		foreach($array as $key => $val) {
			...
			$update = "";
			if(version_compare($this->tem->get_tem_version($val['skin_file']),$this->tem->get_txt_version($val['skin_file'])) === -1) {
				
				$update = "<span class=\"line\">|</span>
						<a href=\"{$_M['url']['own_name']}c=temtool&a=doupdate&no={$val['skin_file']}\" data-confirm=\"您确定要更新此模板吗。\">更新</a>";
			}
			$list[] = "
	...
	public function doupdate()
	{
		global $_M;
		$no = $_M['form']['no'];
		if($msg = $this->tem->dosql($no))
		{
			turnover("{$_M['url']['own_name']}c=temtool&a=dotemlist",'更新成功');
		}else{
			turnover("{$_M['url']['own_name']}c=temtool&a=dotemlist",'数据不存在');
		}
	}
