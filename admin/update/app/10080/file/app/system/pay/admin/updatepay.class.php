...
class updatepay extends admin{
    ...
    public function __construct()
    {
        ...
        //$sql = file_get_contents(__DIR__ . "/{$this->name}_{$this->appver}_{$this->appno}_mysql.json");
        /*#die("{$this->name}_{$this->appver}_{$this->appno}_mysql.json");
    ...
    public function dogetTablesjson()
    {
        ...
        $time = time();
        file_put_contents(__DIR__ . "/{$this->name}_{$this->appver}_{$this->appno}_mysql_new.json", $tables);
        die();
    }
    ...
    {
        global $_M;
        $tablesql = file_get_contents(PATH_SYS . "pay/admin/sql/{$this->name}_{$this->appver}_{$this->appno}_mysql.json");
    ...
    public function insertAppList()
    {
        ...
        if($result['ver'] < $this->appver) {
            $query = "DELETE FROM {$_M['table']['applist']} WHERE no = '{$this->appno}' ";
            DB::query($query);
            $query = "INSERT INTO  {$_M['table']['applist']} SET no = '{$this->appno}',ver = '{$this->appver}',m_name = '{$this->m_name}',m_class = '{$this->m_class}',m_action = '{$this->m_action}',appname = '{$this->appname}',target='0',info  = '{$this->info}',mlangok = '{$this->mlangok}',depend = '{$this->depend}'";
            DB::query($query);
        }
    }
    ...
    public function appconfiginsert($name,$value,$lang)
    {
        ...
        $res = DB::get_one($query);
        if (!$res) {
            $query = "INSERT INTO {$_M['table']['app_config']} SET `name`='{$name}', `value`='{$value}' ,lang='{$lang}'";
            DB::query($query);
        }
    }
    ...
    public function updatePayApi()
    {
        ...
        $paypal_config = DB::get_all($query);
        $query = "DELETE FROM {$_M['table']['config']} WHERE `name`='paypal_config'";
        DB::query($query);
        foreach ($paypal_config as $val) {
            $query = "SELECT * FROM {$_M['config']['tablepre']}pay_api WHERE `name`='paypal_config' AND `lang`='{$val['lang']}'";
            $res = DB::get_one($query);
            if(!$res){
                $query = "INSERT INTO {$_M['config']['tablepre']}pay_api SET `name`='paypal_config' ,`paytype`='5' ,`value`='{$paypal_config['value']}',`lang`='{$val['lang']}'";
                DB::query($query);
            }
        }
    }
    ...
    public function setWebLang()
    {
        ...
        foreach ($_M['langlist']['web'] as $lang) {
            ...
            if ($lang['mark'] == 'cn' || $lang['mark'] == 'en') {
                $content = file_get_contents(PATH_ALL_APP . "pay/admin/sql/lang_web_{$this->appno}_{$lang['mark']}.json");
                $content = jsondecode($content);
                $this->lang_insert($content, $lang['mark']);
            }else{
                //其他语种
                $content = file_get_contents(PATH_ALL_APP . "pay/admin/sql/lang_web_{$this->appno}_cn.json");
                $content = jsondecode($content);
                $this->lang_insert($content, $lang['mark']);
            }
        }
    ...
    public function setAdminLang()
    {
        ...
        foreach ($_M['langlist']['admin'] as $lang) {
            if ($lang['mark'] == 'cn' || $lang['mark'] == 'en') {
                $content = file_get_contents(PATH_SYS . "pay/admin/sql/lang_admin_{$this->appno}_{$lang['mark']}.json");
                $content = jsondecode($content);
                $this->lang_insert($content, $lang['mark']);
            }else{
                //其他语种
                $content = file_get_contents(PATH_SYS . "pay/admin/sql/lang_admin_{$this->appno}_cn.json");
