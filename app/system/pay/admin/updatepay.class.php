...
class updatepay extends admin{
    ...
    {
        global $_M;
        $tablesql = file_get_contents(PATH_SYS . "pay/admin/sql/{$this->appname}_{$this->appver}_{$this->appno}_mysql.json");
    ...
    public function insertAppList()
    {
        ...
        if(!$result) {
            $query = "INSERT INTO  {$_M['table']['applist']} SET no = '{$this->appno}',ver = {$this->appver},m_name = '{$this->appname}',m_class = '{$this->m_class}',m_action = '{$this->m_action}',appname = '{$this->name }',target='0',info  = '{$this->info }',mlangok = '{$this->mlangok}',depend = '{$this->depend}'";
            DB::query($query);
        }
    }
    ...
    {
        global $_M;
        $query = "UPDATE {$_M['config']['tablepre']}pay_api SET `lang`='{$_M['config']['met_index_type']}'";
        DB::query($query);
