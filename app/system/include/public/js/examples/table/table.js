	 			...
	 			window.datatableOption=function(obj){
		            ...
		            var language=langset!='cn'&&langset!='en'?'cn':langset,
		            	option={
							...
							language: { // 语言配置文件
								url: pubjspath + 'js/examples/table/lang/'+language+'.php'
							},
							ajax: {
							...
							drawCallback: function(settings){// 表格重绘后回调函数
					            ...
					            if($('[data-original]',this).length){
					            	...
					            	window.M=[];
					            	M['weburl']=siteurl;
					            	window.met_lazyloadbg=M['weburl'] +'public/ui/v2/static/img/loading.gif';
					            	require.async(siteurl+'public/ui/v2/static/plugin/jquery.lazyload.min.js',function(a){
